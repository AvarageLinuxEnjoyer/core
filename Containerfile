...
chmod +x /usr/libexec/xeniaos-group-fix ; 
echo -e '[Unit] ; 
...
WantedBy=default.target multi-user.target' > /usr/lib/systemd/system/xeniaos-group-fix.service  ; 
  ; 
echo -e "enable xeniaos-group-fix.service" > /usr/lib/systemd/system-preset/01-xeniaos-group-fix.preset  ; 
systemctl enable xeniaos-group-fix.service  ; 
  ; 
